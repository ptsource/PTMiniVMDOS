...
int countpaths(char *pathstring);
...
static sendom(char *s, longword towho, word num, byte dtype);
...
static int Sdomain(char *mname, byte dtype, unpacker_funct unpacker,
      void *data, int adddom, longword nameserver, int *timedout);
 ...
 */
static void typeptr_unpacker( char *data,
                     struct rrpart *rrp, struct useek *qp)
{
