...
void stats(){
    ...
    printf("Sent        : %lu \n", sent );
    printf("Received    : %lu \n", received );
    if (sent)        printf("Success     : %lu %\n", (100L*received)/sent);
    if (!received)   printf("There was no response from %s\n", name );
    else {
	temp = ( tot_delays * 2813L)/(512L*received);
...
void main(int argc, char **argv) {
    ...
    if ( argc < 2 )  help();  /* e qui esco dal programma */
    ...
    else      itts = sequence_mode ? 0xffffffffL : 1;

    if (sequence_mode) printf(" once per_second");
    
    printf("\n\n");

    if (!sequence_mode) tot_timeout = (longword)set_timeout( itts + 10 );

    _arp_resolve( host , NULL );   /* resolution it before timer starts */

    
    do {          /* once per second - do all this */
	...
	if ((timer = _chk_ping( host , &new_rcvd)) != 0xffffffffL) {
	    ...
	    last_rcvd = new_rcvd;
	    if (!is_new_line && !debug_on) printf(", ");
	    printf("PING receipt # %lu : response time %lu.%02lu \
